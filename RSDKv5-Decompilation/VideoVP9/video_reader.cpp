...
struct VpxVideoReaderStruct {
  ...
  size_t buffer_size;
  size_t frame_size;
  RSDK::FileInfo* info_retro;
};

VpxVideoReader* vpx_video_reader_open_retro(RSDK::FileInfo* info)
{
    char header[32] = {0};
    VpxVideoReader* reader = NULL;

    if(RSDK::ReadBytes(info, header, 32) != 32)
    {
        RSDK::PrintLog(RSDK::PRINT_NORMAL, "File header on IVF can't be read.");
        return NULL;
    }
    if(memcmp(kIVFSignature, header, 4) != 0)
    {
        RSDK::PrintLog(RSDK::PRINT_NORMAL, "The IVF signature is wrong.");
        return NULL;
    }
    if(mem_get_le16(header + 4) != 0)
    {
        RSDK::PrintLog(RSDK::PRINT_NORMAL, "The IVF uses the wrong IVF version.");
        return NULL;
    }

    reader = (VpxVideoReader*)calloc(1, sizeof(*reader));
    if (!reader)
    {
        RSDK::PrintLog(RSDK::PRINT_NORMAL, "Can't allocate VpxVideoReader");
        return NULL;
    }

    reader->info_retro = info;
    reader->file = info->file;
...
void vpx_video_reader_close_retro(VpxVideoReader *reader) {
  if (reader) {
    //fclose(reader->file);
    free(reader->buffer);
    free(reader);
...
int vpx_video_reader_read_frame_retro(VpxVideoReader *reader) {
  return !ivf_read_frame_retro(reader->info_retro, &reader->buffer, &reader->frame_size,
                         &reader->buffer_size);
}
